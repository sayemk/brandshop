...
//Category Filter at Shop Model
	

	$('#categoryFilter').on( 'change', function (event) {
		var cat_id=$('#categoryFilter').val();
		$(".row.products_list").click(function(event) {
			return false;
		});
		$('#ballsWaveG').show();	
		
		var href ='/shop/index/'+cat_id;
		window.location='/brandshop/shop/index/'+cat_id;
...
jQuery(document).ready(function() { 
	jQuery.ajax({
	  ...
	  success: function(data, textStatus, xhr) {
	    jQuery('#cart_total').html('BDT '+data)
	  },
...
jQuery('body').on('click change', '.minus, .plus, #quantity', function(event) {
	event.preventDefault();
	/* Act on the event */
	var unt_price = $("#unit_price").val();
	var quantity = $("#quantity").val();
	$("#total_price").val(unt_price*quantity)
...
jQuery('body').on('click change', '.minus, .plus, #quantity', function(event) {
	...
	var unt_price = $(this).parents('tr').children(".cart_unit_price").html();
	var quantity = $(this).parents('div').children(".qty").val();
	$("#cart_total_price").val(unt_price*quantity);
	//console.log(unt_price);
	var unt_price = parseFloat(unt_price);
	var quantity = parseFloat(quantity);
	var subTotal=unt_price*quantity;
	$(this).parents('tr').children(".cart_subtotal_price").html('BDT '+ subTotal);
	
	var values = $('td.cart_subtotal_price').map(function() {
   		return $(this).text();
	}).get();
	var total=0;
	...
	//$('table').parents('tr').children("#cart_total").html(total);
	$("th#cart_total").html('BDT '+total+'.00');

	clearTimeout(timeout);
	timeout=setTimeout(function() {
		$.ajax({
			url: '/brandshop/cart/update/',
			type: 'GET',
...
$(document).on('click', '.deleteRow', function(event){
        ...
        if(conf){
			$.ajax({
				url: '/brandshop/cart/remove/'+rowid,
			...
			.fail(function() {
				alert('Opps! Fail To Remove Product');
			})
			.always(function() {
				console.log("complete");
			});
...
jQuery(document).ready(function() {  
	...
	var toplineH = jQuery('.top_line').height();
	var menublockH = jQuery('.menu_block').height();
	
	jQuery('.top_line').css('margin-bottom',menublockH + 'px');
	jQuery('.menu_block').css('top',toplineH + 'px');
	...
	if (jQuery('.top_line').size() && fixed_menu == true) {
		setInterval(scrolled_menu, 10);
	}
});
...
function scrolled_menu() {
	...
	if (jQuery(window).scrollTop() > toplineH) {
		jQuery('.menu_block').addClass('menu_fixed');
	} else {
		jQuery('.menu_block').removeClass('menu_fixed');
	}
};
...
function minPageHeight() {
	...
	$('.page_box').css('min-height', body_h);
}
